...
export default function ProductDetailPage() {
  ...
  if (productQuery.isLoading) {
    return <section className="mx-auto max-w-7xl px-5 py-14 lg:px-8">Loading product...</section>;
  }

  if (!product) {
    return <section className="mx-auto max-w-7xl px-5 py-14 lg:px-8">Product not found.</section>;
  }
    ...
    <section className="mx-auto max-w-7xl px-5 py-14 lg:px-8">
      ...
      <div className="mt-8 grid gap-10 lg:grid-cols-[0.9fr_1.1fr] lg:items-center">
        ...
        <div>
          ...
          <h1 className="mt-6 font-display text-6xl font-black leading-none tracking-[-0.06em] text-soil md:text-8xl">
            {product.name}
          </h1>
          <p className="mt-6 max-w-2xl text-lg leading-8 text-soil/66">
            {product.description} Built into the white-label catalog model with category, variants, images and stock support.
          </p>
          <div className="mt-7 flex flex-wrap gap-3">
            {product.tags.map((tag) => (
              <span key={tag} className="rounded-full bg-white px-4 py-2 text-sm font-bold text-soil/62 shadow-card">
                {tag}
              </span>
            ))}
          </div>
          <div className="mt-10 flex flex-col gap-5 rounded-[2rem] border border-soil/10 bg-white p-6 shadow-card sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-soil/45">
                <span className="line-through">{formatMoney(product.mrp)}</span> / {product.unit}
              </p>
            </div>
            <button
              onClick={() => {
